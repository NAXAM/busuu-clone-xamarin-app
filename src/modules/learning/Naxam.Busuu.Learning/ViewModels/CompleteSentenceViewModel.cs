...
{
    public class CompleteSentenceViewModel : MemoriseBaseViewModel
    {
        ILearningService learningService;
        public CompleteSentenceViewModel(ILearningService learningService)
        {
        ...
        protected async override void InitFromBundle(IMvxBundle parameters)
        {
            ...
            Item = await learningService.GetUnitByType(UnitModel.UnitType.CompleteSentence);
            Image = Item.Images?.Count > 0 ? Item.Images[0] : "";
            Audio = Item.Audios?.Count > 0 ? Item.Audios[0].Link : "";
        }

        public async override void RunNextCommand()
        {
            if (!IsCompleted)
                return;
